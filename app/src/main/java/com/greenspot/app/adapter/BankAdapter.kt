...
) : RecyclerView.Adapter<BankAdapter.ProductViewHolder>() {
    private var count: Int = 0
    private var data: List<BankList> = ArrayList()
...
//    }
    ...
    override fun onBindViewHolder(holder: ProductViewHolder, position: Int) {
        ...
        var selectbankName = helperlang!!.LoadStringPref(AppConfig.PREFERENCE.BANKNAME, "")


        if (selectbankName.equals(data.get(position).bankname)) {
    ...
    fun swapData(data: List<BankList>) {
        this.data = data
        notifyDataSetChanged()
    }
